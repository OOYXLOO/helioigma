...
The current build now has a visible four-phase progress strip, a first-screen phase objective strip with a compact Turing cue line, score carry-over, streak scoring, shift counting, a local best-score readout, a shareable completion receipt code, phase scoring ledger, a human-readable Judge run summary, a Nightfall report for failed manual runs, and a final replay screen that summarizes the run. It also exposes live node-control buttons below the canvas, an in-canvas first-action hint (`Tap nodes or press 1-9. H = hint, D = demo.`), a pre-start canvas preview (`START NODE 1 x3 -> SOL`), a first-move grace coach on `Start Run` that keeps the timer full until the first node shift, a phase-guided manual `Hint` path, tactile node pulse feedback after hints or shifts, optional default-off Web Audio cues, a low-motion `?calm=1` route, system reduced-motion support, a short phase banner plus screen-reader phase announcer when each phase starts, a live Rotor Trace panel, judge shortcut links, a stable `Demo Solve` receipt path, and an optional `?demo=1` auto-demo URL, so judges can play or verify on mobile or desktop without guessing the canvas hit zones. That gives judges a complete loop in one short play session instead of an open-ended prototype.
...
Player feel matters as much as proof: each phase gives a readable target, a visible `SOL -> XOR -> LUX -> BIN` rotation cue, a short timer, immediate node pulse feedback, a changing Hint/Rotor Trace scan order, optional audio, and a clean Nightfall recovery. Daylight is the shared resource: it is the countdown, the mistake cost for exploratory shifts, and the pressure behind the score. The verifier supports the game loop; it does not replace the feeling of making and correcting moves under daylight pressure.
...
- Open `?calm=1` for the same game and stable receipt path with particle bursts and CSS motion reduced; the game also honors system reduced-motion preference.
...
- On 390px mobile, the browser smoke now requires the playable canvas to fit in the first viewport instead of merely showing a partial wheel.
...
- The judge shortcut row stays focused on the strongest five routes: Auto demo, No storage, Judge pack, Verify sample, and the current captioned WebM. Deeper proof links such as calm review, smoke test, GIF fallback, source, manifest, README, and rubric scorecard live inside the Judge pack so the playable page does not turn into a link wall.
...
- Before a run starts, the canvas previews `START NODE 1 x3 -> SOL`; after `Start Run`, the first-move coach on `Start Run` switches to an in-canvas `TRY NODE 1 x3 -> SOL` callout tied to the highlighted node, so a rushed mobile or desktop judge sees the required tap count before the timer starts.
...
- `tools/browser-smoke-check.mjs`: optional clean-clone Playwright check after `npm ci` that opens the real pages, verifies first-screen Hint, Demo Solve, `Start Run`, pre-start first-move preview, first-action grace, default-off Audio controls, score/receipt explanation, daylight meter, desktop canvas priority, mobile Run Path after the canvas, phase objective strip, Turing cue copy, Nightfall report, phase announcer, phase scoring ledger, Judge run summary, award signals, Rotor Trace, mobile canvas visibility, `?calm=1` and system reduced-motion review paths, validates the run receipt and receipt-verifier query path, fails on external network requests, console errors, or page errors, and waits for `smoke.html` to reach 71 PASS checks.
