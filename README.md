...
- Before a run starts, the canvas previews `START NODE 1 x3 -> SOL`; after `Start Run`, the first-move coach switches to an in-canvas `TRY NODE 1 x3 -> SOL` callout tied to the highlighted node, so a rushed mobile or desktop judge sees the required tap count before the timer starts.
