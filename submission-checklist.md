...
- Run `npm run audit:launch` after packaging for local readiness. Before the public repo exists, run `npm run audit:launch:public`; the expected status is `WAIT_USER_GATE` with the repo/Pages/DEV next steps, not a local package failure.
...
- Manifest check: `judge-manifest.json` records the Best Ode to Alan Turing category, 70 expected smoke checks, sample receipt verifier URL, score basis, Nightfall recovery, and stable receipt `SC-4P-2907-62-Y5VFX1`.
...
- Screenshots/media: use the public Pages URLs for `cover.png`, `helioigma-demo.webm`, `helioigma-demo.gif`, `desktop-check-v5.png`, `mobile-check-v6.png`, `desktop-complete-v4.png`, and `mobile-complete-v1.png`.
