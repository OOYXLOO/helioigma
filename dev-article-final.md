...
The project is deliberately small: `index.html`, `styles.css`, and `game.js` hold the playable game; `proof-verifier.html`, `smoke.html`, `judge.html`, `RUBRIC_SCORECARD.md`, and `judge-manifest.json` make the review path inspectable; `tools/build-demo-video.mjs`, `tools/build-demo-webm.mjs`, and `tools/build-demo-gif.py` reproduce the media; `tools/verify-dev-article.mjs` checks that this article and the judge manifest agree on public links, media versions, and launch boundaries; `tools/verify-media-freshness.mjs` checks PNG, GIF, and WebM freshness; and `LICENSE` is the MIT license for the game package.
